            ...
            <div class="flex justify-between h-24 items-center">
                ...
                <div class="hidden sm:flex space-x-8">
                    ...
                    <a href="{{ route('services') }}" wire:navigate
                        class="hover:text-orange-500 relative group">Servicios
                        <span
                            class="block h-0.5 bg-orange-500 absolute bottom-0 left-0 w-0 group-hover:w-full transition-all duration-300"></span>
                    </a>
                    <a href="{{ route('sites') }}" wire:navigate
                        class="hover:text-orange-500 relative group">Sitios
                        <span
                            class="block h-0.5 bg-orange-500 absolute bottom-0 left-0 w-0 group-hover:w-full transition-all duration-300"></span>
                    </a>
                    @auth
                        <a href="{{ route('plans') }}" wire:navigate class="hover:text-orange-500 relative group">Planes
                            <span
                                class="block h-0.5 bg-orange-500 absolute bottom-0 left-0 w-0 group-hover:w-full transition-all duration-300"></span>
                        </a>
                    @else
                        <a href="{{ route('about') }}" wire:navigate
                            class="hover:text-orange-500 relative group">Conocenos
                            <span
                                class="block h-0.5 bg-orange-500 absolute bottom-0 left-0 w-0 group-hover:w-full transition-all duration-300"></span>
                        </a>
                    @endauth
                        ...
                        <x-dropdown align="right" width="48">
                            <x-slot name="trigger">
                                ...
                                @else
                                    <span class="inline-flex rounded-md">
                                        <button type="button"
                                            class="inline-flex text-right items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none focus:bg-gray-50 active:bg-gray-50 transition ease-in-out duration-150">
                                            {{ Auth::user()->email }}
                            ...
                            <x-slot name="content">
                                ...
                                <form method="POST" action="{{ route('logout') }}" x-data wire:navigate>
                                    ...
                                    <x-dropdown-link href="#" wire:navigate @click.prevent="$root.submit();">
                                        {{ __('Cerrar sesion') }}
                                    </x-dropdown-link>
                                </form>
                            </x-slot>
                ...
                @else
                    <div class="flex items-center space-x-2">
                        <a href="{{ route('login') }}" wire:navigate
                            class="focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500 hover:text-orange-500 relative group">
                            Inicia Sesion
                            <span
                                class="block h-0.5 bg-orange-500 absolute bottom-0 left-0 w-0 group-hover:w-full transition-all duration-300"></span>
                        </a>
